...
#             # Pad the remaining area with zeros (black) to form the full image frame
...
# def embed_data_pvd(cover_image_path: str, data_bits: str, out_path: str) -> int:
#     """
...
# def decode_data_pvd(stego_path: str, max_bits_to_decode: int) -> str:
#     """Decodes data embedded with PVD from the Red Channel."""
...
#     # Iterate over non-overlapping pixel pairs (p'_i, p'_{i+1})
#     for i in range(height):
#         for j in range(0, width - 1, 2):
#             if len(extracted_bits) >= max_bits_to_decode:
#                 break 

#             p_i_prime = int(arr_r[i, j])
#             p_i1_prime = int(arr_r[i, j+1])

#             # 1. Calculate Stego Difference (d')
#             d_prime = abs(p_i1_prime - p_i_prime)
            
#             # 2. Determine Range and Bits
#             lk, uk, t = _get_pvd_range(d_prime)
#             if t == 0: continue
            
#             # Determine actual bits to extract
#             t_actual = min(t, max_bits_to_decode - len(extracted_bits))
#             if t_actual == 0:
#                 break
                
#             # 3. Extract Decimal Value (b)
#             # b = d' - lk
#             b = d_prime - lk
            
#             # 4. Convert to Binary (t_actual bits)
#             # The value 'b' must be represented by 't' bits, then truncated to t_actual
#             extracted_chunk_bin = format(b, f'0{t}b')[:t_actual]
#             extracted_bits += extracted_chunk_bin
...
def open_pvd_dashboard(root, back_callback):
    ...
    app = StegApp(root)   # ← use your PVD GUI class here
